...
int main(int argc, char **argv) {
   ...
   off_t baddr_dg_brdg = 0;
   char *endptr;
   
   struct sockaddr_in address;

   while ((c = getopt(argc, argv, "vd:p:a:")) != -1)
      switch (c) {
      case 'v':
         verbose = 1;
         break;
	  ...
	  case 'p':
		 ...
		 break;
	  case 'a':
	          baddr_dg_brdg = (off_t)strtol(optarg, &endptr, 16);
			  if (endptr == optarg || *endptr != '\0') {
				fprintf(stderr,"Failed Addr\n");
				return -1;
			  }
	          break;
      case '?':
         fprintf(stderr, "usage: %s [-v] [-d <uio_to_be_used>] [-p <port>] [-a <address>]\n example: %s -v -d mem -p 2542 -a 0x40000000\n", *argv ,*argv);
         return 1;
      }
    printf("port = %d\n", port);
	strcat(dev_file,d);
